...
export const payments = pgTable('payments', {
    ...
    id_user: uuid('id_user').references(() => users.id_user).notNull(), // FK al usuario que paga
...
export const disbursements = pgTable('disbursements', {
    ...
    trip_id: varchar('trip_id', { length: 255 }).notNull(),
    amount: decimal('amount', { precision: 12, scale: 2 }).notNull(),
    id_user: uuid('id_user').references(() => users.id_user).notNull(), // FK al usuario que recibe
...
export const refunds = pgTable('refunds', {
    ...
    trip_id: varchar('trip_id', { length: 255 }).notNull(),
    amount: decimal('amount', { precision: 12, scale: 2 }).notNull(),
    id_user: uuid('id_user').references(() => users.id_user).notNull(), // FK al usuario devuelto
