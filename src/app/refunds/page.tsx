...
export default async function RefundsPage({ searchParams }: DashboardProps) {
	...
	const paymentsUser = await ReadCookieUserInformation("/refunds");
	...
	const hasNextPage = userRefunds.length > ITEMS_PER_PAGE;

	// Recortamos el array para quedarnos únicamente con los 5 correspondientes a la página actual
	const displayRefunds = userRefunds.slice(0, ITEMS_PER_PAGE);
		...
		<div className="min-h-screen bg-slate-50 text-slate-900 pb-16">
			...
			<div className="bg-white border-b border-slate-200 py-8 mb-10">
				<div className="container mx-auto px-4 max-w-4xl">
					<TransactionControlPanel type="refunds"/>
					...
					<p className="text-sm text-slate-500 mt-1">
						Historial de devoluciones filtradas cronológicamente.
					</p>
				</div>
			</div>
			...
			<main className="container mx-auto px-4 max-w-4xl">
				<div className="w-full flex flex-col items-center gap-6">
					...
					<div className="w-full flex flex-col items-center gap-6">
						{displayRefunds.map((tx) => (
							<Link
								href={`/refunds/${tx.trip_id}`}
								key={tx.trip_id}
                ...
                */}
								<div className="w-full transition-all duration-200 hover:-translate-y-1 active:scale-[0.99] group-hover:shadow-xl rounded-2xl">
									<TransactionCard
										...
										type={"REFUND"}
										refundType={tx.refund_type || "No especificada"} // Información adicional para reembolsos
									/>
								</div>
							</Link>
						))}
					...
					{displayRefunds.length === 0 && (
						<div className="text-center py-16 bg-white rounded-2xl border border-slate-200 border-dashed w-full">
							<p className="text-slate-500 font-medium">No se encontraron devoluciones en esta página.</p>
							{currentPage > 1 && (
								<Link href="/refunds?page=1" className="text-blue-600 text-sm font-semibold hover:underline mt-2 inline-block">
					...
					{userRefunds.length > 0 && (
						<div className="w-full flex items-center justify-between border-t border-slate-200 pt-6 mt-4">
							...
							{currentPage > 1 ? (
								<Link
									href={`/refunds?page=${currentPage - 1}`}
