...
export default async function DisbursementsPage({ searchParams }: DashboardProps) {
		...
		<div className="min-h-screen bg-slate-50 text-slate-900 pb-16">
			...
			<div className="bg-white border-b border-slate-200 py-8 mb-10">
				<div className="container mx-auto px-4 max-w-4xl">
					<TransactionControlPanel type="disbursements"/>
					...
					<p className="text-sm text-slate-500 mt-1">
						Historial de liquidaciones filtrados cronológicamente.
					</p>
				</div>
			</div>
			...
			<main className="container mx-auto px-4 max-w-4xl">
				<div className="w-full flex flex-col items-center gap-6">
					...
					<div className="w-full flex flex-col items-center gap-6">
						{displayDisbursements.map((tx) => (
							<Link
								href={`/disbursements/${tx.trip_id}`}
								key={tx.trip_id}
                ...
                */}
					...
					{displayDisbursements.length === 0 && (
						<div className="text-center py-16 bg-white rounded-2xl border border-slate-200 border-dashed w-full">
							<p className="text-slate-500 font-medium">No se encontraron desembolsos en esta página.</p>
							{currentPage > 1 && (
								<Link href="/disbursements?page=1" className="text-blue-600 text-sm font-semibold hover:underline mt-2 inline-block">
					...
					{displayDisbursements.length > 0 && (
						<div className="w-full flex items-center justify-between border-t border-slate-200 pt-6 mt-4">
							...
							{currentPage > 1 ? (
								<Link
									href={`/disbursements?page=${currentPage - 1}`}
